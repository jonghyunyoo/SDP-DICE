...
$if not set gridn_m     $set gridn_m 5
$if not set gridn_p     $set gridn_p 6
$if not set gridn_u     $set gridn_u 5
$if not set gridn_l     $set gridn_l 2
$if not set gridn_ot    $set gridn_ot 3

$eval gridn (max(%gridn_k%,%gridn_m%,%gridn_u%,%gridn_l%,%gridn_ot%,%gridn_p%))

* Parameter definition for DICE 2016R
$eval t_last (%niter%)
...
$setglobal isn        ik,im,iu,il,ip,iot
$setglobal jsn        jk,jm,ju,jl,jp,jot
$setglobal ksn        kk,km,ku,kl,kp,kot
...
lo("CAP") = 0.2;
lo("PHI") = (2*(0.1)-up("PHI")-up("PHI")*(smin(ip,x_p(ip))+(1e-15)))/(1-(smin(ip,x_p(ip))+(1e-15)));
lo("OT")  = (2*(0.0068)-up("OT")-up("OT")*(smin(iot,x_ot(iot))+(1e-15)))/(1-(smin(iot,x_ot(iot))+(1e-15)));
lo("MAT") = (2*(mate/mate)-up("MAT")-up("MAT")*(smin(im,x_m(im))+(1e-15)))/(1-(smin(im,x_m(im))+(1e-15)));
lo("MU")  = lo("MAT")*(1-b11)/b21;
...
display lev_k, lev_m, lev_mu, lev_ml, lev_p, lev_ot, lev_kt;
...
stoc_bm(sgrid(%isn%),ig) = sqrt(2*lev_bs_init)*ghdata(ig,"zeta") + (1-lambda/lev_s_init);
...
sccdef_val(ct(t),%isn%)$(sgrid(%isn%) and t.val ne card(t))..
                         ESTSCC_valfn(t,%isn%) =e= -1000*(ESTGRAD_M(t,%isn%)/3.666/mate)/((ESTGRAD_K(t,%isn%)+1e-6)/lbar_t(t)*abar_t(t)**(1/(gama-1)));
...
MAT.LO(t+1,sgrid(%isn%))   = lo("MAT")*1.01;
PHIT.LO(t+1,sgrid(%isn%),ig,ih)  = 1e-6;
...
* Iteration Log
        tulog(t)                        = TU(t,"1","1","1","1","1","1");
        itlog("K_N",t,sgrid(%isn%))     = K_N(%isn%);
        itlog("Y",t,sgrid(%isn%))       = Y(t,%isn%);
        itlog("I",t,sgrid(%isn%))       = I(t,%isn%);
        itlog("C",t,sgrid(%isn%))       = C(t,%isn%);
        itlog("K",t,sgrid(%isn%))       = K(%isn%);
        itlog("EF_K",t,sgrid)           = EF_K(t+1,sgrid);
        itlog("MAT",t,sgrid)            = MAT(t+1,sgrid)*mate;
        itlog("lev_mat",t,sgrid(%isn%)) = lev_m(im)*mate ;
        itlog("MIU",t,sgrid)            = MIU(t,sgrid);
        itlog("Ygross",t,sgrid(%isn%))  = YGROSS(t,%isn%);
        itlog("E",t,sgrid(%isn%))       = E(t,%isn%);
        itlog("lev_kt",t,sgrid(%isn%))  = lev_kt(t,ik) ;
        itlog("delta_K",t,sgrid)        = itlog("K",t,sgrid)-itlog("lev_kt",t,sgrid);
        itlog("delta_M",t,sgrid(%isn%)) = itlog("MAT",t,sgrid)-lev_m(im)*mate;
        itlog("Inv_share",t,sgrid)      = itlog("I",t,sgrid)/itlog("Y",t,sgrid);
        itlog2("PHIT",t,sgrid,ig,ih)    = PHIT(t+1,sgrid,ig,ih);
        itlog("SCC",t,sgrid(%isn%))     = ESTSCC(t,%isn%);
        itlog("CPRICE",t,sgrid(%isn%))  = ((pback*(1-gback)**(t.val-1))*MIU.l(t,%isn%)**(expcost2-1));
$offdotl

errorlog("error_K",t(t_exc_last),sgrid)        = yes$(itlog("EF_K",t,sgrid) ge up("CAP") or itlog("EF_K",t,sgrid) le lo("CAP"));
errorlog("error_MAT",t(t_exc_last),sgrid)      = yes$(itlog("MAT",t,sgrid)/mate ge up("MAT") or itlog("MAT",t,sgrid)/mate le lo("MAT"));
errorlog2("error_PHIT",t(t_exc_last),sgrid,ig,ih) = yes$(itlog2("PHIT",t,sgrid,ig,ih) ge up("PHI") or itlog2("PHIT",t,sgrid,ig,ih) le lo("PHI"));
errorlog3(t) = yes$((bellman_reduced.solvestat ne 1) or (bellman_reduced.modelstat ne 2));

abort$(sum((universal,t(t_exc_last5),sgrid), errorlog(universal,t,sgrid)) gt 0) 'Check the optimal solution';
...
$macro PRE_ST_PV  ( sum(ccgrid(%jsn%), coef_t(%jsn%) *\
                         K_sim_CS(PRE_K_N) * MAT_sim_CS(PRE_MAT_N) * MU_sim_CS(PRE_MU_N) * ML_sim_CS(PRE_ML_N) * PHIT_sim_CS(PRE_PHIT_N) * OT_sim_CS(PRE_OT_N)))

$macro PST_ST_PV  ( sum(ccgrid(%jsn%), coef_tn(%jsn%) *\
                         K_sim_CS(PST_K_N) * MAT_sim_CS(PST_MAT_N) * MU_sim_CS(PST_MU_N) * ML_sim_CS(PST_ML_N) * PHIT_sim_CS(PST_PHIT_N) * OT_sim_CS(PST_OT_N)))
...
$macro unc_bm       (sqrt(2*PRE_BSIG)*ghdata(ig,"zeta")+PRE_BMIU)
...
bmiuneq(ig,ih).. PST_BMIU(ig,ih) =E= PRE_BMIU;
bsigneq..        PST_BSIG =E= PRE_BSIG ;
bseq(ig,ih)..    PST_S(ig,ih) =E= lambda/(1-PST_BMIU(ig,ih));
...
PST_BMIU.fx(ig,ih) = PRE_BMIU; # No Learning
PST_BSIG.fx = PRE_BSIG;  # No Learning
...
$ondotl
         ...
         log_sim(sim_iter,te,"bnd_err_UP") = Yes$(PRE_MAT gt smax(im,lev_m(im)) or PRE_EFK gt smax(ik,lev_k(ik)) or PRE_PHIT gt smax(ip,lev_p(ip)) or PRE_MU gt smax(iu,lev_mu(iu)) or PRE_ML gt smax(il,lev_ml(il)) );
         log_sim(sim_iter,te,"bnd_err_LO") = Yes$(PRE_MAT lt smin(im,lev_m(im)) or PRE_EFK lt smin(ik,lev_k(ik)) or PRE_PHIT lt smin(ip,lev_p(ip)) or PRE_MU lt smin(iu,lev_mu(iu)) or PRE_ML lt smin(il,lev_ml(il)) );
